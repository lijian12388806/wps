...
import com.liveneo.plat.exception.LoginNameNotExistException;
import com.liveneo.plat.exception.LoginNamePasswordNotMatchException;
...
public class UserinfoService extends AbstractBaseService implements UserinfoServiceIface{
	...
	public Boolean checkUsername(Userinfo user){
		...
		DetachedCriteria detachedCriteria = DetachedCriteria
		.forClass(Userinfo.class);
		detachedCriteria.add(Restrictions.eq(Userinfo.PROP_USERNAME,user.getUsername()));
	...
	public Boolean checkAgentno(Userinfo user){
		...
		DetachedCriteria detachedCriteria = DetachedCriteria
		.forClass(Userinfo.class);
		detachedCriteria.add(Restrictions.eq(Userinfo.PROP_AGENTNO,user.getAgentno()));
		list=this.UserinfoDAO.findByCriteria(detachedCriteria);
		if(user.getId() == null){
			if(list.size()>0){
				b=false;
	...
	public List<Userinfo> findUserinfoBySql(DetachedCriteria detachedCriteria, int pageNo, int pageSize) {
		return this.UserinfoDAO.findByCriteria(detachedCriteria, pageNo, pageSize);
	}
//	public List<Userinfo> findUserinfoBySql(DetachedCriteria detachedCriteria, int pageNo, int pageSize) {
//		return this.UserinfoDAO.findByCriteria(detachedCriteria, pageNo, pageSize);
//	}
	...
	public Userinfo getBy(String propertyName, Object propertyVal) {
		...
		return UserinfoDAO.getBy(propertyName, propertyVal);
	}
	
	
	public Userinfo login(String loginName, String password){
		Userinfo userinfo = (Userinfo) getBy("loginName", loginName);
		
		if(userinfo == null){
			throw new LoginNameNotExistException();
		}else{
			if(!userinfo.getUserpwd().equals(password)){
				throw new LoginNamePasswordNotMatchException();
			}
		}
		
		//登录次数 + 1
//		userinfo.setVisitedTimes(userinfo.getVisitedTimes() + 1); 
