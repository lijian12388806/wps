...
public class WorkReportAction extends AbstractActionSupport {
	...
	public String listWorkratioReport(){
		...
		querySql.append("SELECT IFNULL(c.totalcount,0) tcount,IFNULL(c.closecount,0) ccount,u.USERNAME username,IFNULL(c.cti,0) totaltime FROM userinfo u LEFT JOIN ");
		querySql.append("(");
		querySql.append("select a.ac totalcount,IFNULL(b.bc, 0) closecount ,a.ap puser,a.ati cti from (");
		querySql.append("select count(*) ac ,pk_userinfo ap,sum((unix_timestamp(DEALTIME)-unix_timestamp(CALLTIME))/60) ati from  workorder ");
		...
		querySql.append("(");
		querySql.append("select count(*) bc,pk_userinfo bp,sum((unix_timestamp(DEALTIME)-unix_timestamp(CALLTIME))/60) bti from WORKORDER where WOSTATUS = 'yjd' ");
