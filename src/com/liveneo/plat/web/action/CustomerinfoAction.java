...
public class CustomerinfoAction extends AbstractActionSupport {
	...
	public String listCustomerinfo() {
		...
		List<Customerinfo> l = customerinfoService.findCustomerBySql(getHql(
	...
	private String getHql(int startIndex, int numPerPage) {
		...
		if (StringUtils.isNotEmpty(querycustphoneno))
			hql = (new StringBuilder(String.valueOf(hql))).append(
					" and custphoneno like '%").append(querycustphoneno)
					.append("%'").toString();
		if (StringUtils.isNotEmpty(queryinfosource))
			hql = (new StringBuilder(String.valueOf(hql))).append(
					" and temp2 = '").append(queryinfosource).append("'")
					.toString();
		if (StringUtils.isNotEmpty(querytemp1))
			hql = (new StringBuilder(String.valueOf(hql))).append(
					" and Temp1 like '%").append(querytemp1).append("%'").toString();
		if (StringUtils.isNotEmpty(queryemailaddr))
			hql = (new StringBuilder(String.valueOf(hql))).append(
					" and Emailaddr like '%").append(queryemailaddr).append("%'")
					.toString();
		...
		try {
			...
			if (StringUtils.isNotEmpty(cusphonelistKey)) {
				...
				this.logService(getRequest())
						.addLog(this.getRequest(), "LOGTYPE-XG",
								cusphonelist.toString(), Cusphonelist.REF);
			} else {
				WebappUtil.copyProperties(cusphonelist, cusphonelistForm);
				cusphonelist.setPkCustomerinfo(customer);
				cusphonelist.setSynstatus("Nsyn");
				...
				// this.customerinfoService.updateCustphoneno(customer);
				this.logService(getRequest())
						.addLog(this.getRequest(), "LOGTYPE-XZ",
								cusphonelist.toString(), Cusphonelist.REF);
			}
			this.getSession().setAttribute("operationDone", "ajaxDone");
			return ajaxForwardSuccess(getText("common.saveok"));
	...
	public String listCusphonelist() {
		customerKey = this.getRequest().getParameter("customerKey");
		int pageNum = IntegerUtil.converStrToInteger(this.getPageNum()) > 0 ? IntegerUtil
				.converStrToInteger(this.getPageNum()) - 1
				: 0;
		int numPerPage = IntegerUtil.converStrToInteger(getNumPerPage());
		...
		List<Cusphonelist> l = this.customerinfoService
				.findCusphonelistBySql(getCustSql(startIndex, numPerPage));
	...
	private String getCustSql(int startIndex, int numPerPage) {
		String sql = "select * from Cusphonelist where PK_CUSPHONELIST >0 ";
		if (StringUtils.isNotEmpty(querylinkman))
			sql = (new StringBuilder(String.valueOf(sql))).append(
					" and Custlinkname like '%").append(querylinkman).append(
					"%'").toString();
		if (StringUtils.isNotEmpty(querynostatus))
			sql = (new StringBuilder(String.valueOf(sql))).append(
					" and phoneno like '%").append(querynostatus).append("%'")
					.toString();
		sql += " and PK_CUSTOMERINFO = " + customerKey;
		sql = (new StringBuilder(String.valueOf(sql))).append(" limit ")
				.append(startIndex).append(",").append(numPerPage).toString();
		return sql;
	}
	...
	private DetachedCriteria getCusDetachedCriteria() {
		...
		if (StringUtils.isNotEmpty(querynostatus)) {
			detachedCriteria.add(Restrictions.ilike(Cusphonelist.PROP_PHONENO,
					querynostatus, MatchMode.ANYWHERE));
		}
		detachedCriteria.add(Restrictions.eq(Cusphonelist.PROP_PK_CUSTOMERINFO,
				this.customerinfoService.getCustomer(IntegerUtil
						.converStrToInteger(customerKey))));
		return detachedCriteria;
	}
